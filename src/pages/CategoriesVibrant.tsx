...
export default function CategoriesVibrant() {
  ...
  const ALL_CATEGORIES = [
    ...
    { id: '19', name: t('Online εκδηλωσεις', 'Online events'), icon: Monitor, count: 42, color: 'text-gray-900', bg: 'bg-gray-50', border: 'border-gray-200' },
    ...
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-900 font-medium text-[13px] mt-1.5">{t('Εξερευνήστε όλο το φάσμα των εμπειριών μας.', 'Explore our full range of curated experiences.')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-3">
        {visibleCategories.map(cat => (
          ...
          >
            ...
            <p className="text-[10px] font-semibold text-gray-900 mt-1 capitalize tracking-wide">{cat.count} {t('Εκδηλώσεις', 'Events')}</p>
          </div>
        ))}
      </div>
      ...
      {visibleCategories.length === 0 && (
         <div className="text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
           <p className="text-gray-900 font-medium text-sm">{t('Δεν βρέθηκαν κατηγορίες.', 'No categories found matching ')} "{searchQuery}".</p>
         </div>
      )}
            ...
            <div className="flex gap-1 shrink-0 bg-gray-100/80 p-0.5 rounded-lg border border-gray-200/50">
              {['All', 'Free', 'Paid', 'Group Discount'].map((price) => (
                <button
                  ...
                  className={`text-[10px] sm:text-xs font-semibold px-2.5 sm:px-3 py-1.5 rounded-md transition-all whitespace-nowrap ${priceFilter === price ? 'bg-white text-cyan-700 shadow-sm border border-gray-200/50 ring-1 ring-black/5' : 'text-gray-900 hover:text-gray-900 hover:bg-gray-200/50'}`}
                >
            ...
            <div className="flex gap-1 shrink-0 bg-gray-100/80 p-0.5 rounded-lg border border-gray-200/50">
              {['Any', 'Today', 'This Week', 'This Month'].map((date) => (
                <button
                  ...
                  className={`text-[10px] sm:text-xs font-semibold px-2.5 sm:px-3 py-1.5 rounded-md transition-all whitespace-nowrap ${dateFilter === date ? 'bg-white text-cyan-700 shadow-sm border border-gray-200/50 ring-1 ring-black/5' : 'text-gray-900 hover:text-gray-900 hover:bg-gray-200/50'}`}
                >
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {isLoading ? (
            ...
            <div className="col-span-1 sm:col-span-2 lg:col-span-3 text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
              <p className="text-gray-900 font-medium text-sm">{t('Δεν βρέθηκαν εκδηλώσεις για τα κριτήριά σας.', 'No events found matching your criteria.')}</p>
            </div>
          ) : (
      ...
      <div className="mt-8 bg-cyan-50 rounded-2xl p-6 border border-cyan-100 flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-900 mt-1">{t('Δημιουργήστε τη δική σας εκδήλωση και συγκεντρώστε άτομα με παρόμοια ενδιαφέροντα.', 'Create your own event and gather people with similar interests.')}</p>
        </div>
