...
export default function OrganizerProfileBento() {
  ...
  if (!organizer) return <div className="p-8 text-center text-gray-900 font-medium">{t('Ο διοργανωτής δεν βρέθηκε', 'Organizer not found')}</div>;
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-8 shadow-sm text-center sm:text-left sm:flex sm:items-start gap-8">
        ...
        <div className="mt-6 sm:mt-0 flex-1 space-y-4">
          <div>
            ...
            <p className="text-sm text-gray-900 font-medium mt-1"><MapPin className="h-3.5 w-3.5 inline mr-1" /> {organizer.city}</p>
          </div>
          
          <p className="text-sm text-gray-900 leading-relaxed max-w-2xl">
          ...
          <div className="flex items-center gap-4 bg-gray-50 p-3 rounded-lg border border-gray-100 w-fit">
             <div className="flex items-center gap-1 text-yellow-500">
               {[1, 2, 3, 4, 5].map(star => (
                 <svg key={star} className={`w-4 h-4 ${star <= 4 ? 'fill-current' : 'text-gray-900'}`} viewBox="0 0 24 24">
             ...
             <div className="text-xs font-bold text-[#111827]">
               4.2 / 5.0 
               <span className="text-gray-900 font-medium ml-1">({t('120+ Κριτικές Εκδηλώσεων', '120+ Event Reviews')})</span>
             </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 pt-2">
            <button 
              ...
              className="text-[10px] font-bold tracking-wide text-gray-900 hover:text-cyan-600 transition-colors flex items-center gap-1"
            >
      ...
      <div className="space-y-4">
        ...
        {hostedEvents.length === 0 ? (
          <div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-200">
            <p className="text-gray-900 font-medium text-sm">{t('Δεν έχουν προγραμματιστεί επερχόμενες εκδηλώσεις.', 'No upcoming events currently scheduled.')}</p>
          </div>
        ) : (
              ...
              <Link key={event.id} to={`/events/${event.id}`}>
                <Card className="flex items-start gap-4 p-4 hover:border-cyan-300 transition-colors cursor-pointer group">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-900 mt-1">{event.time} • {event.locationArea}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowContactModal(false)} className="text-gray-900 hover:text-gray-900 transition-colors">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="flex justify-end gap-3">
              <button 
                ...
                className="px-4 py-2 text-sm font-bold text-gray-900 hover:bg-gray-100 rounded-lg transition-colors"
              >
                {t('Ακύρωση', 'Cancel')}
              </button>
              <button 
