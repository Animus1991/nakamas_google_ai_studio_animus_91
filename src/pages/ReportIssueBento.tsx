    ...
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-900 font-medium text-[13.551608211075px] md:text-[16.25212883329px] mt-1">{t(`Βοηθήστε μας να κρατήσουμε την κοινότητα ασφαλή`, `Help us keep the community safe`)}</p>
        </div>
        <button onClick={() => navigate(-1)} className="text-gray-900 hover:text-[#111827] transition-colors rounded-full p-2 bg-white border border-gray-200 shadow-sm">
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      {step === 1 ? (
          ...
          <form onSubmit={handleSubmit} className="p-4 md:p-6 space-y-5">
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-gray-200 rounded-xl p-4 text-center hover:border-cyan-300 hover:bg-cyan-50/30 transition-colors cursor-pointer">
                <Upload className="w-5 h-5 text-gray-900 mx-auto mb-1" />
                <p className="text-[12.5px] text-gray-900 font-medium">{t(`Ανεβάστε screenshots ή φωτογραφίες`, `Upload screenshots or photos`)}</p>
                <p className="text-[10px] text-gray-900 mt-0.5">PNG, JPG {t(`έως`, `up to`)} 5MB</p>
              </div>
            </div>

            <div className="bg-gray-50 rounded-lg p-4 flex items-start gap-3">
              <ShieldCheck className="w-5 h-5 text-cyan-600 shrink-0 mt-0.5" />
              <p className="text-[13.5px] text-gray-900 font-medium leading-relaxed"><span className="font-bold text-[#111827]">{t(`Απόρρητο:`, `Privacy:`)}</span> {t(`Η αναφορά σας είναι εμπιστευτική.`, `Your report is confidential.`)}</p>
            </div>

            {/* Expected response time */}
            <div className="flex items-center gap-2 justify-center text-[12.5px] text-gray-900 font-medium">
              <Clock className="w-3.5 h-3.5" />
              <span>{t(`Αναμενόμενος χρόνος απόκρισης: <24 ώρες`, `Expected response time: <24 hours`)}</span>
            </div>

            <button type="submit" className="w-full bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 text-gray-900 py-2.5 rounded-full text-[12.15px] font-bold shadow-sm hover:bg-black transition-colors  tracking-wider">{t(`Υποβολή Αναφοράς`, `Submit Report`)}</button>
          </form>
        </div>
      ) : (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
          ...
          <p className="text-[18px] text-gray-900 max-w-md mx-auto mb-6">{t(`Ευχαριστούμε. Θα εξετάσουμε την αναφορά σας και θα σας ενημερώσουμε.`, `Thank you. We'll review your report and get back to you.`)}</p>
          <button 
            onClick={() => navigate('/')}
