  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div>
        ...
        <p className="text-gray-900 font-medium text-[13.551608211075px] md:text-[14.626916949961px] mt-1">{t(`Πείτε μας πώς ήταν η εμπειρία σας`, `Tell us about your experience`)}</p>
      </div>
        ...
        {step === 1 && (
          <div className="space-y-8">
              ...
              <div className="flex gap-3 justify-center">
                {moods.map(m => (
                  ...
                  >
                    ...
                    <span className="text-[10.90125px] font-bold text-gray-900">{m.label}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-[14.535px] text-gray-900 font-medium mb-3">{t(`Πώς θα βαθμολογούσατε την εκδήλωση;`, `How would you rate the event?`)}</p>
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button key={star} onClick={() => setOverallRating(star)} className="p-2 hover:bg-gray-50 rounded-full transition-colors">
                    <Star className={`w-8 h-8 transition-colors ${star <= overallRating ? 'text-amber-400 fill-amber-400' : 'text-gray-900 hover:text-amber-400'}`} />
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-[14.535px] text-gray-900 font-medium mb-3">{t(`Πώς ήταν η ατμόσφαιρα;`, `How was the atmosphere?`)}</p>
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button key={`vibe-${star}`} onClick={() => setVibeRating(star)} className="p-2 hover:bg-gray-50 rounded-full transition-colors">
                    <Star className={`w-8 h-8 transition-colors ${star <= vibeRating ? 'text-cyan-400 fill-cyan-400' : 'text-gray-900 hover:text-cyan-400'}`} />
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-[15px] text-gray-900 font-medium mb-3">{t(`Ήταν όλοι παρόντες;`, `Was everyone present?`)}</p>
              <div className="space-y-3 mt-4">
                <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                  <input type="radio" name="attendance" className="h-4 w-4 text-cyan-600 rounded-full border-gray-300" />
                  <span className="text-[16.2px] font-medium text-gray-900">{t(`Ναι, ήρθαν όλοι`, `Yes, everyone showed up`)}</span>
                </label>
                <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                  <input type="radio" name="attendance" className="h-4 w-4 text-cyan-600 rounded-full border-gray-300" />
                  <span className="text-[16.2px] font-medium text-gray-900">{t(`Όχι, κάποιοι δεν ήρθαν`, `No, some people didn't show up`)}</span>
                </label>
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-[15px] text-gray-900 font-medium mb-3">{t(`Νιώσατε ασφαλείς;`, `Did you feel safe?`)}</p>
              <div className="space-y-3 mt-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
              />
              <p className="text-[11.2px] text-gray-900 font-medium mt-1">{t(`Αυτό θα είναι ορατό στον διοργανωτή`, `This will be visible to the organizer`)}</p>
            </div>
        ...
        {step === 2 && (
          <div className="space-y-6 text-center py-8">
            ...
            <div>
              ...
              <p className="mt-2 text-[16.2px] text-gray-900 font-medium leading-relaxed max-w-sm mx-auto">{t(`Η αξιολόγησή σας βοηθά στη βελτίωση της εμπειρίας για όλους.`, `Your feedback helps improve the experience for everyone.`)}</p>
            </div>
