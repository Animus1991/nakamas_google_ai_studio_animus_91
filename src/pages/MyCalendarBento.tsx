    ...
    <div className="max-w-full mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in pb-20 md:pb-0">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-900 font-medium text-xs md:text-sm mt-1">{t('Διαχειριστείτε το πρόγραμμά σας και τις προσεχείς εμπειρίες.', 'Manage your schedule and upcoming experiences.')}</p>
        </div>
        
        <div className="flex bg-gray-100 p-1 rounded-lg">
          <button 
            className={`px-4 py-1.5 rounded-md text-xs font-bold transition-colors ${view === 'week' ? 'bg-white shadow-sm text-[#111827]' : 'text-gray-900'}`}
            onClick={() => setView('week')}
          >
            {t('Εβδομάδα', 'Week')}
          </button>
          <button 
            className={`px-4 py-1.5 rounded-md text-xs font-bold transition-colors ${view === 'month' ? 'bg-white shadow-sm text-[#111827]' : 'text-gray-900'}`}
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setCurrentDate(new Date())} className="px-3 py-1 text-xs font-bold bg-gray-100 text-gray-900 rounded-md hover:bg-gray-200">
              {t('Σήμερα', 'Today')}
            </button>
              ...
              <div className="grid grid-cols-7 gap-2 mb-4">
                {days.map((day, i) => (
                  <div key={i} className="text-center">
                    <div className="text-[10px] font-bold text-gray-900 tracking-wide">{format(day, 'EEE')}</div>
                ...
                {upcomingEvents.map((event, idx) => (
                  <div key={idx} className="flex gap-4 p-4 rounded-xl border border-gray-100 hover:border-cyan-100 hover:bg-cyan-50/30 transition-colors cursor-pointer" onClick={() => navigate(`/events/${event.id}`)}>
                    <div className="flex flex-col items-center justify-center w-16 shrink-0 border-r border-gray-100 pr-4">
                      <span className="text-xs font-bold text-gray-900">{format(event.parsedDate, 'MMM')}</span>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-gray-900 font-medium">
            ...
            >
              ...
              <div className="grid grid-cols-7 gap-1 mb-1">
                {[t('Δευ', 'Mon'), t('Τρι', 'Tue'), t('Τετ', 'Wed'), t('Πεμ', 'Thu'), t('Παρ', 'Fri'), t('Σαβ', 'Sat'), t('Κυρ', 'Sun')].map(d => (
                  <div key={d} className="text-center text-[10px] font-bold text-gray-900 tracking-wide py-2">{d}</div>
                ))}
              </div>
              ...
              {upcomingEvents.length > 0 && (
                <div className="mt-4 space-y-2 border-t border-gray-100 pt-4">
                  <h4 className="text-[10px] font-bold text-gray-900 tracking-wide mb-2">{t('Προσεχώς αυτόν τον μήνα', 'Upcoming This Month')}</h4>
                  {upcomingEvents.map((event, idx) => (
                    <div
                      key={idx}
                    ...
                    >
                      <div className="text-[10px] font-bold text-gray-900 w-12 shrink-0">
